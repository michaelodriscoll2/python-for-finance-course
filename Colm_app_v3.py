...
sl.title(f"Risk metrics - AAPL")
sl.subheader(f"Return Metrics")
c1, c2, = sl.columns(2)
c1.metric("Mean Daily Return", f"{pctmean_daily_return:.2}%")
c2.metric("Annual Standard Deviation", f"{sd_pct:.2}%")

sl.subheader("Risk and Risk adjusted Return")
c3, c4, = sl.columns(2)
c3.metric("Sharpe Ratio", f"{sharpe:.2}")
c4.metric("Value at Risk Annual, 95 confidence", f"{vatr_annual_pct:.2f}%")