...
if ticker == "":
    ...
    st.stop()
add_to_portfolio = st.checkbox("Add this ticker to portfolio", value=False)
...
#Zach's part
st.subheader("Expected Portfolio Returns Dashboard")
...
if ticker_returns.empty or SPX_returns.empty:
    ...
    st.write(f"Daily Returns for {ticker} (last {show_days} days):")
    if ma_window > 0:
        st.line_chart(df_plot[[ticker, f"{ticker}_MA"]])
    else:
        st.line_chart(df_plot[[ticker]])
...
st.write(f"Expected Annual Return: {optimal_portfolio_returns:.4f}")
st.write(f"Expected Volatility: {optimal_portfolio_volatility:.4f}")
st.write(f"Optimal Sharpe Ratio: {optimal_sharpe_ratio:.4f}")
