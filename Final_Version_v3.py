...
st.markdown(f"[Click here]({ft_search})")
...
#Zach's part
st.subheader("Daily Returns & CAPM Analysis for " + ticker)
...
# risk-free rate input - US 10 year government bond yield fetched live

def get_10y_gov_bond_yield():
    """Return US 10y Treasury Yield as decimal or None on failure."""
    try:
        df_yield = yf.download("^TNX", period="5d", interval="1d", progress=False, auto_adjust=False)
        if df_yield is None or df_yield.empty:
            return None
        val = float(df_yield["Adj Close"].iloc[-1])
        # convert percentage to decimal
        rf_decimal = val / 100.0 if val > 1 else val
        return rf_decimal
    except Exception:
        return None

# call the function 
live_rf = get_10y_gov_bond_yield()
if live_rf is not None:
    st.metric("Live US Treasury 10 year bond yield (risk-free rate)", f"{live_rf:.2%}")
    rf = float(live_rf)

else: # in the event of failure
    st.caption("Live US Treasury 10 year bond yield unavailable — please enter risk-free rate manually.")

    # allow user to override the fetched value if needed
    rf = st.number_input("Risk-Free Rate (annual)", min_value=0.0, max_value=1.0,
